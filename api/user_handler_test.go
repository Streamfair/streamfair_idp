		...
		},
		{
			...
			buildStubs: func(store *mock_db.MockStore) {
				store.EXPECT().
					...
					Times(1).
					Return(db.IdpSvcUser{}, sql.ErrConnDone)
			},
			checkResponse: func(recorder *httptest.ResponseRecorder) {
		...
		},
		{
			...
			buildStubs: func(store *mock_db.MockStore) {
				store.EXPECT().
					CreateUser(gomock.Any(), gomock.Any()).
					Times(1).
					Return(db.IdpSvcUser{}, &pgconn.PgError{Code: "23505", Message: "duplicate key value violates unique constraint"})
			},
			checkResponse: func(recorder *httptest.ResponseRecorder) {
...
func randomUser(t *testing.T) (user db.IdpSvcUser, password string) {
...
func requireBodyMatchUser(t *testing.T, body *bytes.Buffer, user db.IdpSvcUser) {
	data, err := io.ReadAll(body)
	require.NoError(t, err)

	var gotUser db.IdpSvcUser
